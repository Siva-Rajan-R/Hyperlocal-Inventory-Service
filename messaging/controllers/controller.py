...
from ..consumers.products_consumer import ProductsConsumer
...
from ..consumers.prod_purchase_consumer import ProductPurchaseConsumer
from ..consumers.supp_purchase_consumer import SupplierPurchaseConsumer
...
class ConsumersHandler:
    ...
    async def main_handler(msg:AbstractIncomingMessage):
        ...
        ic(f"Name => {domain}\nWork-For => {work_for}\nAction => {action}\nstate => {state}\nVersion => {version}")
        if f"{domain}.{work_for}"=='products.inventory':
            await ConsumersHandler._helper(consumer_cls=ProductsConsumer,msg=msg,service_name=SERVICE_NAME.lower(),exchange_service_name=domain,readdb_cls=ReadDbInventoryService)
        
        elif f"{domain}.{work_for}"=='products.purchase':
            await ConsumersHandler._helper(consumer_cls=ProductPurchaseConsumer,msg=msg,service_name=SERVICE_NAME.lower(),exchange_service_name=domain)
        
        elif f"{domain}.{work_for}"=='suppliers.purchase':
            await ConsumersHandler._helper(consumer_cls=SupplierPurchaseConsumer,msg=msg,service_name=SERVICE_NAME.lower(),exchange_service_name=domain)
